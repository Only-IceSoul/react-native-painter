...
class RadialGradientView :  PaintableView {
    
    
    private var mLayer = RadialGradientLayer()
    ...
    @objc func setW(_ v:NSNumber?){
        let ev = CGFloat(truncating: v ?? 0)
        mLayer.setW(ev.clampNotNegative())
    }
    @objc func setH(_ v:NSNumber?){
        let ev = CGFloat(truncating: v ?? 0)
        mLayer.setH(ev.clampNotNegative())
    }
    ...
    @objc func setRx(_ v:NSNumber?){
        let ev = CGFloat(truncating: v ?? 0)
        mLayer.setRx(ev.clampNotNegative())
    }
    @objc func setRy(_ v:NSNumber?){
        let ev = CGFloat(truncating: v ?? 0)
        mLayer.setRy(ev.clampNotNegative())
    }
