...
const RadialGradientWeb =  React.forwardRef((props,ref)=>{
    ...
    return(
        <g ref={myRef} opacity={op}  mask={`url(#${mk})`}  transform={transform}  
         ...
         curradiusx={radiusX} curradiusy={radiusY}
        >
...
class RadialGradient extends React.PureComponent{
    ...
    handleProps(k,value){
        switch(k){
            ...
            case "cx":
            case "cy":
                this.updateAttribute(Attributes[k],value,this.getElement(Attributes.elements.gradient))
                return true
            case "rx":
                this.updateAttribute(Attributes.r,value,this.getElement(Attributes.elements.gradient))
                let curRy = parseFloat(this.getAttribute(Attributes.curRadiusY,this.myRef.current))
                this.updateAttribute(Attributes.gradientTransform
                    ,`translate(0.5 0.5) scale(1,${curRy/value}) translate(-0.5 -0.5)`,
                    this.getElement(Attributes.elements.gradient))
                    this.updateAttribute(Attributes.curRadiusX,value,this.myRef.current)
                return true
            case "ry":
                let curRx = parseFloat(this.getAttribute(Attributes.curRadiusX,this.myRef.current))
                this.updateAttribute(Attributes.gradientTransform
                    ,`translate(0.5 0.5) scale(1,${value/curRx}) translate(-0.5 -0.5)`,
                    this.getElement(Attributes.elements.gradient))
                    this.updateAttribute(Attributes.curRadiusY,value,this.myRef.current)
                return true
            case "colors":
    ...
    render(){
        return <RadialGradientWeb ref={this.myRef} {...this.props} />
    }
}
